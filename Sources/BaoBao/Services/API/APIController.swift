...
class APIController: NSObject {
    ...
    func registerMessageHandlers(for webView: WKWebView) {
        ...
        userContentController.add(self, name: "favoriteStory")
        userContentController.add(self, name: "unfavoriteStory")
        userContentController.add(self, name: "isStoryFavorited")
        userContentController.add(self, name: "getFavoriteStories")
...
extension APIController: WKScriptMessageHandler {
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        case "favoriteStory":
            handleFavoriteStory(webView: webView, body: body, callbackID: callbackID)
            
        case "unfavoriteStory":
            handleUnfavoriteStory(webView: webView, body: body, callbackID: callbackID)
            
        case "isStoryFavorited":
            handleIsStoryFavorited(webView: webView, body: body, callbackID: callbackID)
            
        case "getFavoriteStories":
            handleGetFavoriteStories(webView: webView, body: body, callbackID: callbackID)
    ...
    private func handlePlayAudio(webView: WKWebView, body: [String: Any], callbackID: String) {
        ...
        speechService.playAudio(from: audioURL) { [weak self] success in
            ...
            if success {
                ...
                let error = NSError(domain: "com.baobao.app", code: 500, userInfo: [NSLocalizedDescriptionKey: "音频播放失败"])
                self.sendResponse(to: webView, callbackID: callbackID, data: nil, error: error)
            }
        }
    }
    ...
    private func handleSaveStory(webView: WKWebView, body: [String: Any], callbackID: String) {
        ...
        // 获取可选参数
        let id = storyDict["id"] as? String ?? UUID().uuidString
    ...
    private func handleGetStoryDetail(webView: WKWebView, body: [String: Any], callbackID: String) {
        ...
        if let story = dataService.getStory(withId: storyId) {
            ...
            if let audioURL = story.audioURL {
                storyDict["audioURL"] = audioURL.absoluteString
            }
    ...
    // 处理收藏故事请求
    private func handleFavoriteStory(webView: WKWebView, body: [String: Any], callbackID: String) {
        // 解析参数
        guard let params = body["params"] as? [String: Any],
              let storyId = params["storyId"] as? String else {
            logger.error("❌ 收藏故事参数无效")
            let error = NSError(domain: "com.baobao.app", code: 400, userInfo: [NSLocalizedDescriptionKey: "参数无效"])
            sendResponse(to: webView, callbackID: callbackID, data: nil, error: error)
            return
        }
        ...
        // 收藏故事
        let success = dataService.favoriteStory(withId: storyId)
    ...
    // 处理取消收藏故事请求
    private func handleUnfavoriteStory(webView: WKWebView, body: [String: Any], callbackID: String) {
        // 解析参数
        guard let params = body["params"] as? [String: Any],
              let storyId = params["storyId"] as? String else {
            logger.error("❌ 取消收藏故事参数无效")
            let error = NSError(domain: "com.baobao.app", code: 400, userInfo: [NSLocalizedDescriptionKey: "参数无效"])
            sendResponse(to: webView, callbackID: callbackID, data: nil, error: error)
            return
        }
        ...
        // 取消收藏故事
        let success = dataService.unfavoriteStory(withId: storyId)
    ...
    // 处理检查故事是否已收藏请求
    private func handleIsStoryFavorited(webView: WKWebView, body: [String: Any], callbackID: String) {
        // 解析参数
        guard let params = body["params"] as? [String: Any],
              let storyId = params["storyId"] as? String else {
            logger.error("❌ 检查故事收藏状态参数无效")
            let error = NSError(domain: "com.baobao.app", code: 400, userInfo: [NSLocalizedDescriptionKey: "参数无效"])
            sendResponse(to: webView, callbackID: callbackID, data: nil, error: error)
            return
        }
        ...
        // 检查故事是否已收藏
        let isFavorite = dataService.isStoryFavorited(withId: storyId)
        
        // 发送响应
        sendResponse(to: webView, callbackID: callbackID, data: ["isFavorite": isFavorite])
    }
    
    // 处理获取收藏故事列表请求
    private func handleGetFavoriteStories(webView: WKWebView, body: [String: Any], callbackID: String) {
        // 获取收藏的故事列表
        let favoriteStories = dataService.getFavoriteStories()
        
        // 转换为字典数组
        let storiesData = favoriteStories.map { $0.toDictionary() }
        
        // 发送响应
        sendResponse(to: webView, callbackID: callbackID, data: ["stories": storiesData])
    }
